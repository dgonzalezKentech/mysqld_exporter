...
func (ScrapeNdbinfoDiskpagebuffers) Scrape(ctx context.Context, instance *instance, ch chan<- prometheus.Metric, logger *slog.Logger) error {
	db := instance.getDB()
	ndbinfoDiskpagebuffersRows, err := db.QueryContext(ctx, ndbinfoDiskpagebuffersQuery)
	if err != nil {
		logger.Error("Error querying ndbinfo.diskpagebuffers", "err", err)
		return err
	}
