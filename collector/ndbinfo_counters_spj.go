...
func (ScrapeNdbinfoCountersSPJ) Scrape(ctx context.Context, instance *instance, ch chan<- prometheus.Metric, logger *slog.Logger) error {
	db := instance.getDB()
	ndbinfoCountersSPJRows, err := db.QueryContext(ctx, ndbinfoCountersSPJQuery)
	if err != nil {
		logger.Error("Error querying ndbinfo.counters.spj", "err", err)
		return err
	}
