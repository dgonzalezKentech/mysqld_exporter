...
func (ScrapeNdbinfoDiskWriteSpeedAggregate) Scrape(ctx context.Context, instance *instance, ch chan<- prometheus.Metric, logger *slog.Logger) error {
	db := instance.getDB()
	ndbinfoDiskWriteSpeedAggregateRows, err := db.QueryContext(ctx, ndbinfoDiskWriteSpeedAggregateQuery)
	if err != nil {
		logger.Error("Error querying ndbinfo.disk_write_speed_aggregate", "err", err)
		return err
	}
